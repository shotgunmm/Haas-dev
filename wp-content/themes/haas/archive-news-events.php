...
<section id="event" data-speed="4" data-type="background">
	...
	<ul class="blog-pagination">
		...
		<li><a class="button" href="<?= get_bloginfo('url') ?>/news-events/?paginate=1">&lt;&lt;</a></li>
		<li><a class="button" href="<?= get_bloginfo('url') ?>/news-events/?paginate=<?= $pagaginate - 1 ?><?= $q ?>">&lt;</a></li>
	<? endif ?>

	<? foreach( get_surrounding_numbers( $pagaginate, 2, 1, $main->max_num_pages ) as $link ) : ?>
	<li><a class="button<? if( $link == $pagaginate ) : ?> active<? endif ?>" href="<?= get_bloginfo('url') ?>/news-events/?paginate=<?= $link ?><?= $q ?>"><?= $link ?></a></li>
<? endforeach ?>

<? if( $main->max_num_pages != $pagaginate ) : ?>
<li><a class="button" href="<?= get_bloginfo('url') ?>/news-events/?paginate=<?= ( $pagaginate + 1 ) ?><?= $q ?>">&gt;</a></li>
<li><a class="button" href="<?= get_bloginfo('url') ?>/news-events/?paginate=<?= ( $main->max_num_pages ) ?><?= $q ?>">&gt;&gt;</a></li>
<? endif ?>
</ul>
<? endif ?>
</div>
