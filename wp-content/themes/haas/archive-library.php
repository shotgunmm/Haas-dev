...
if( isset( $_GET['category'] ) ) $main_args['tax_query'] = array( array( 'taxonomy' => 'library', 'field' => 'slug', 'terms' => $_GET['category'] ) );
...
<?php if(has_post_thumbnail()){
								...
								<div class="col-md-10 title">
									...
									<span class="posted"><?php the_author(); ?></span><br>
									</div>
								<?php }else{ ?>
<div class="title">
									<a href="<?= get_permalink() ?>"><h3><?= get_the_title() ?></h3></a>
								...
								<?php if(has_post_thumbnail()){
									?>
								<div class="lib-content col-md-10">
									<?php 
									//$content = the_excerpt();
									$pos = 15;
								
								echo '<p>' . tiny_excerpt(get_the_excerpt(),12) . '</p>';?>
				...
				<div class="event-container">
					...
					<a href="<?= site_url() ?>/library"><h4>Please expand your search</h4></a>
	...
	<ul class="blog-pagination">
		<? if( $pagaginate != 1 ) : ?>
		<!--<li><a class="button" href="/library<?= $q ? '?' . ltrim( $q, '&' ) : '' ?>">&lt;&lt;</a></li>-->
		<li><a class="button" href="<?= get_bloginfo('url') ?>/library/?paginate=1">&lt;&lt;</a></li>
		<li><a class="button" href="<?= get_bloginfo('url') ?>/library/?paginate=<?= $pagaginate - 1 ?><?= $q ?>">&lt;</a></li>
	<? endif ?>

	<? foreach( get_surrounding_numbers( $pagaginate, 2, 1, $main->max_num_pages ) as $link ) : ?>
	<li><a class="button<? if( $link == $pagaginate ) : ?> active<? endif ?>" href="<?= get_bloginfo('url') ?>/library/?paginate=<?= $link ?><?= $q ?>"><?= $link ?></a></li>
<? endforeach ?>

<? if( $main->max_num_pages != $pagaginate ) : ?>
<li><a class="button" href="<?= get_bloginfo('url') ?>/library/?paginate=<?= ( $pagaginate + 1 ) ?><?= $q ?>">&gt;</a></li>
<li><a class="button" href="<?= get_bloginfo('url') ?>/library/?paginate=<?= ( $main->max_num_pages ) ?><?= $q ?>">&gt;&gt;</a></li>
<? endif ?>
</ul>
<? endif ?>
</div>
