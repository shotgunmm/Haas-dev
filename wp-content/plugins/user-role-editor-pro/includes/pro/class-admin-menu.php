...
class URE_Admin_Menu {
    ...
    protected function user_allowed($cap_required, $allowed_roles, $allowed_caps) {        
        ...
        if ( in_array($cap_required, array('switch_themes', 'customize')) && 
            ...
            return true;    // permissions extension for "Appearance" menu, "Themes", "Cusomize" items of "Appearance" menu
        }
...
<?php
            if (isset($current_submenu[$menu_item[2]])) {
                foreach($current_submenu[$menu_item[2]] as $submenu_item) {
                    if (!$this->user_allowed($submenu_item[1], $allowed_roles, $allowed_caps)) {
